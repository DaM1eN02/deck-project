  ...
  class VanillaTilt {
    constructor(element, settings = {}) {
      if (!(element instanceof Node)) {
        throw ("Can't initialize VanillaTilt because " + element +
          " is not a Node.");
      }
    ...
    addEventListeners() {
      ...
      if (this.glare || this.fullPageListening) {
        document.addEventListener("resize", this.onWindowResizeBind);
      }
    ...
    removeEventListeners() {
      ...
      if (this.glare || this.fullPageListening) {
        document.removeEventListener("resize", this.onWindowResizeBind);
      }
    }
    ...
    onDeviceOrientation(event) {
      ...
      const totalAngleX = this.settings.gyroscopeMaxAngleX -
        this.settings.gyroscopeMinAngleX;
      const totalAngleY = this.settings.gyroscopeMaxAngleY -
      ...
      if (this.fullPageListening) {
        this.event = {
          clientX: (this.settings.startX + this.settings.max) /
            (2 * this.settings.max) * this.clientWidth,
          clientY: (this.settings.startY + this.settings.max) /
            (2 * this.settings.max) * this.clientHeight,
        };
      } else {
        this.event = {
          clientX: this.left +
            ((this.settings.startX + this.settings.max) /
              (2 * this.settings.max) * this.width),
          clientY: this.top +
            ((this.settings.startY + this.settings.max) /
              (2 * this.settings.max) * this.height),
        };
      }
    ...
    resetGlare() {
      if (this.glare) {
        this.glareElement.style.transform = "rotate(180deg)";
    ...
    getValues() {
      ...
      const tiltX =
        (this.reverse * (this.settings.max - x * this.settings.max * 2))
          .toFixed(2);
      const tiltY =
        (this.reverse * (y * this.settings.max * 2 - this.settings.max))
          .toFixed(2);
    ...
    update() {
      ...
      if (this.glare) {
        this.glareElement.style.transform = `rotate(${values.angle}deg)`;
    ...
    updateClientSize() {
      this.clientWidth = document.innerWidth ||
        document.documentElement.clientWidth ||
        document.body.clientWidth;

      this.clientHeight = document.innerHeight ||
    ...
    setTransition() {
      clearTimeout(this.transitionTimeout);
      this.element.style.transition = this.settings.speed + "ms " +
